...
public:
    ...
    void Run(void* ptr)
    {
		...
		bool notLeaf = curNH.first->split(curAlpha, pJD->variance, pEntropy, pJD->mu, pJD->attrIds, pJD->s, pJD->numUsed, pJD->compN);
...
CTree::CTree(double alphaIn, double muIn, int* attrIdsIn, int expectNumIn, int* numUsedIn, double* compNIn, double varianceIn): alpha(alphaIn), mu(muIn), attrIds(attrIdsIn), s(expectNumIn), numUsed(numUsedIn), compN(compNIn), variance(varianceIn), root()
{
}
...
void CTree::grow(bool doFS, idpairv& attrCounts)
{
	...
	while(!nodes.empty())
	{
		...
		bool notLeaf = curNH.first->split(curAlpha, variance, pEntropy, mu, attrIds, s, numUsed, compN);
	...
	while(toDoN > 0)
	{
		...
		if(toDoN > 0)
		{
			...
			JobData* pJD = new JobData(curNH, &nodes, &nodesCond, &toDoN, pAttrCounts, b, H, mu, attrIds, s, numUsed, compN, variance);
