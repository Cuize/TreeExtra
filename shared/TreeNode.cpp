...
{	
	...
	for(int attrNo = 0; attrNo < (int)pAttrs->size();)
	{
		...
		{//boolean attribute
			...
			double eval = evalBool(boolSplit, nodeV, nodeSum, squares) + penalty;
			if(isnan(eval))
		 ...
		 //all splits are calculated in one pass
			...
			{//on each iteration of this cycle collect info about the block of cases with the
				...
				if(!isnan(prevResp) && (prevDiff || curDiff || (prevResp != curResp)))
				{
					...
					double eval = (( sqErr1 + sqErr2 + squares )/variance) + penalty;
...
double CTreeNode::evalBool(SplitInfo& canSplit, double nodeV, double nodeSum, double squares)
{
	...
	return (sqErr1 + sqErr2 + squares)/(squares - nodeSum*nodeSum/nodeV);
}
